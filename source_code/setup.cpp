...
STDAPI RegisterFilters( BOOL bRegister )
{
    ...
    if(bRegister)
    { 
        hr = AMovieSetupRegisterServer(CLSID_PushSourceDesktop, L"avisynth-script-capture-source", achFileName, L"Both", L"InprocServer32");
    }
    ...
    if( SUCCEEDED(hr) )
    {
        ...
        if( SUCCEEDED(hr) )
        {
            if(bRegister)
            {
				...
				// this name actually shows up in VLC
                hr = fm->RegisterFilter(CLSID_PushSourceDesktop, L"avisynth-script-capture-source", &pMoniker, &CLSID_VideoInputDeviceCategory, NULL, &rf2);
            }
            else
            {
